...
public:
	...
	*/
	...
	static JSON toJSON(const RemoveResponseMessage &remove_response) {
		...
		j[MESSAGE_ADD] = toJSON(remove_response.remove);
		return j;
	}
	/**
	...
	*/
	...
	static RemoveResponseMessage parseRemoveResponse(const JSON &remover) {
		RemoveMessage remove = parseRemove(remover[MESSAGE_ADD]);
		std::string status = remover[MESSAGE_STATUS];
		std::string info = remover[MESSAGE_INFO];
